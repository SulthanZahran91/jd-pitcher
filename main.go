...
func main() {
	...
	// Static files (no rate limit — accessible to all)
	...
	r.Group(func(r chi.Router) {
		...
		r.Post("/api/pitch", func(w http.ResponseWriter, r *http.Request) {
			...
			logRequest(r, logger, cfg, req.JD, pitch, "success")
			resp := PitchResponse{
				Pitch:     pitch,
		...
		r.Post("/pitch", func(w http.ResponseWriter, r *http.Request) {
			...
			logRequest(r, logger, cfg, jd, pitch, "success")

			data := internal.ResultData{
				Name:      cfg.Profile.Name,
				PitchHTML: template.HTML(nl2br(template.HTMLEscapeString(pitch))),
			}

			out, err := internal.RenderResult(resultTmpl, data)
			if err != nil {
...
func logRequest(r *http.Request, logger *internal.Logger, cfg *internal.Config, jd, pitch, status string) {
	ip, _, _ := net.SplitHostPort(r.RemoteAddr)
	if ip == "" {
		ip = r.RemoteAddr
	}
	if err := logger.Log(ip, jd, pitch, cfg.LLMModel, status); err != nil {
